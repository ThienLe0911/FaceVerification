...
if DEPENDENCIES_AVAILABLE:
    ...
    def simulate_face_detection(image_path: Path) -> Dict[str, Any]:
        """Real face detection using MTCNN (same as preprocessing) with simulation fallback"""
        try:
            print(f"🔍 Face detection for {image_path.name}")
            
            # Try MTCNN first (same as preprocessing.py)
            try:
                from facenet_pytorch import MTCNN
                from PIL import Image as PILImage
                
                print(f"   Using MTCNN detector (same as preprocessing)")
                
                # Initialize MTCNN with same settings as preprocessing
                ...
                mtcnn = MTCNN(
                    ...
                    post_process=True,
                    device=device,
                    keep_all=True  # Get all faces with confidence
                )
                ...
                if boxes is not None and len(boxes) > 0:
                    ...
                    # Filter faces by confidence (high confidence only)
                    valid_faces = []
                    for i, (box, prob) in enumerate(zip(boxes, probs)):
                        if prob > 0.9:  # High confidence only
                            ...
                            area_ratio = face_area / (width * height)
                            
                            # Quality based on size and confidence
                            if area_ratio > 0.1:  # Face > 10% of image
                                quality = min(95, int(70 + prob * 25))
                            elif area_ratio > 0.05:  # Face > 5% of image
                                quality = min(85, int(60 + prob * 25))
                            else:
                                quality = min(75, int(50 + prob * 25))
                            ...
                            print(f"     Face {len(valid_faces)}: confidence={prob:.3f}, quality={quality}")
                        else:
                            print(f"     Face {i+1}: Low confidence={prob:.3f}, filtered")
                    
                    print(f"   ✅ Final: {len(valid_faces)} high-confidence faces")
                    
                    # Return based on face count
                    if len(valid_faces) == 0:
                        return {
                            "status": "no_face",
                            "message": "Không phát hiện khuôn mặt với confidence cao",
                            "faces_count": 0,
                            "quality": 0,
                            "face_locations": [],
                            "method": "mtcnn"
                        ...
                        return {
                            ...
                            "face_locations": [(y1, x2, y2, x1)],  # (top, right, bottom, left)
                            "bbox": face["bbox"],
                            "method": "mtcnn"
                        }
                    else:
                    ...
                    return {
                        "status": "no_face",
                        "message": "MTCNN không phát hiện khuôn mặt",
                        "faces_count": 0,
                        "quality": 0,
                        "face_locations": [],
                        "method": "mtcnn"
                ...
                print("⚠️ MTCNN not available, fallback to simulation")
            except Exception as e:
                print(f"⚠️ MTCNN error: {e}, fallback to simulation")
